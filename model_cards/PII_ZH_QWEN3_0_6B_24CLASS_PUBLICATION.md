...
不是当前发布授权，也不应由使用方改写。发布包的资格与证据闭包来自经过校验的外部 publication
manifest、许可证批准回执、私密安全渠道 evidence 和本地候选回执；公开发布授权仅来自维护者在
远端 draft/private 证据核验后的单独最终确认。本 `0.2.0rc1` 明确使用维护者 waiver receipt：PVR
已启用，但独立合成端到端测试未完成，不得将其表述为渠道测试通过。
...
- 安全问题应通过目标 GitHub 仓库已启用的私密漏洞报告渠道提交。
- 独立渠道测试尚未完成；维护者 `whyiug` 于 2026-07-20 只豁免 `0.2.0rc1` 的该测试门禁。该
  waiver 不关闭 PVR、不提供生产安全保证，也不替代公开发布前的单独最终确认。
