...
def test_positive_and_negative_chinese_field_context() -> None:
    ...
    assert positive.recognition_metadata["context_enhancement"]["positive_matches"] == ["身份证号"]
    assert negative.recognition_metadata["context_enhancement"]["negative_matches"] == ["订单号"]
