...
GitHub draft → pre-publication gate → 单独公开确认”执行。任何一步身份、哈希、可见性或证据不一致
...
- [x] 独立、无管理权限账号的合成 PVR 端到端测试尚未完成；不得声称已验收通过。维护者 `whyiug`
  于 2026-07-20 明确豁免 `0.2.0rc1` 的这一项测试门禁，范围与限制已记录在
  [`PRIVATE_SECURITY_CHANNEL_WAIVER.md`](PRIVATE_SECURITY_CHANNEL_WAIVER.md)。
...
- [x] commit `ab44d8b7b0f7cae2db9bd1bcff4ff2318ba89bd7` 的 hosted CI 已通过；它只是历史
  preflight，不能替代最终 source commit 的 CI。
- [x] 本机 SSH 公钥已登记为 GitHub SSH signing key，并已核验 fingerprint
  `SHA256:j3DjYC57yPDuXhn5qbIKYmc0OLyJrRtbqgvsLPIl4hg`；不得上传或记录私钥。
- [ ] 必须从已提交的 waiver 文档生成仓库外、自哈希、只读的
  `private-security-channel-waiver-receipt.json`。本轮明确走 waiver 路径，不生成或宣称
  tested-channel PASS。
...
export SECURITY_CHANNEL_WAIVER_RECEIPT="$RELEASE_RUN/private-security-channel-waiver-receipt.json"
...
## 3. 私密渠道 waiver 回执与 signing key

### 3.1 本轮使用维护者 waiver

本轮不把独立测试写成已完成，也不读取、复制或公开任何 private advisory 正文或 GHSA 私密标识。
维护者账号自身创建的 draft（如有）不构成独立报告证据。使用已提交的
`PRIVATE_SECURITY_CHANNEL_WAIVER.md` 生成外部自哈希回执。最终回执必须等 source commit 冻结后，
从该 commit 的 clean worktree 用 `uv run --frozen` 生成；不得从当前可变工作树生成。
`WAIVER_AUTHORIZED_AT` 固定为 waiver 文档记录的维护者授权接收时间，两处必须 exact match：

```bash
export WAIVER_AUTHORIZED_AT='2026-07-20T08:29:44Z'
```

该回执必须明确记录 PVR enabled、independent test not completed、maintainer waiver，且只豁免本 RC
的测试门禁。它不是 tested-channel PASS、生产安全保证或最终公开确认。

tested-channel receipt 仍是工具支持的替代路径，但当前提交的 waiver source policy 已将本 RC 锁定到
waiver 路径。若要改用独立账号真实完成的 tested evidence，必须形成一个删除该 waiver policy 的
新 source commit 并重新跑完整 CI/签名/tag 流程；不得在同一 source commit 或同一 tag 内换证据类型。
successor 要求 tested receipt 与 waiver receipt **必须且只能提供一个**；本轮固定提供 waiver receipt。
...
维护者已在浏览器设备页确认，并使用 GitHub REST 将 `~/.ssh/id_ed25519.pub` 登记为 signing key。
不得上传私钥。推送 tag 前仍要通过公开 signing-key endpoint 比较 exact public key，并确认 fingerprint
为固定值；远端状态漂移时立即停止。
...
test "$(gh api --hostname github.com --method GET \
  "repos/${GH_REPO}/private-vulnerability-reporting" --jq .enabled)" = true
...
test ! -e "$BUILD_TREE"
test ! -e "$DIST_DIR"
test ! -e "$SECURITY_CHANNEL_WAIVER_RECEIPT"
...
(
  ...
  uv run --frozen python scripts/build_private_security_channel_waiver_receipt.py \
    --output "$SECURITY_CHANNEL_WAIVER_RECEIPT" \
    --security SECURITY.md \
    --waiver release/community-v2-rc1/PRIVATE_SECURITY_CHANNEL_WAIVER.md \
    --package-version 0.2.0rc1 \
    --github-repository "$GH_REPO" \
    --hugging-face-repository "$HF_REPO_ID" \
    --authorized-by whyiug \
    --authorized-at "$WAIVER_AUTHORIZED_AT" \
...
test "$(stat -c %a "$SECURITY_CHANNEL_WAIVER_RECEIPT")" = 444
test -f "$WHEEL_PATH"
test -f "$SBOM_PATH"
...
COMMON_SUCCESSOR_ARGS=(
  ...
  --private-security-channel-waiver-receipt "$SECURITY_CHANNEL_WAIVER_RECEIPT"
  --git-source-commit "$SOURCE_COMMIT"
  --github-repository "$GH_REPO"
  --hugging-face-repository "$HF_REPO_ID"
)
...
info = HfApi(endpoint='https://huggingface.co', token=os.environ['HF_TOKEN']).repo_info(
    repo, repo_type='model', revision='main'
)
if (
    info.id != repo
    ...
    raise SystemExit('HF target must be the exact private repository before upload')
print(json.dumps({'repo': info.id, 'private': True, 'pre_upload_sha': info.sha}, sort_keys=True))
PY
)
```
...
- private-security-channel waiver receipt SHA-256，并明确 independent test not completed；
...
- [ ] PVR 仍为 enabled；waiver receipt 只覆盖 `0.2.0rc1` 测试门禁，并继续明确 independent test
  not completed，未宣称 tested-channel PASS 或生产安全保证。
