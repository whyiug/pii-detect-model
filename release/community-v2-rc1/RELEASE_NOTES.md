...
> 状态：仅完成本地候选验收，尚未在 GitHub 或 Hugging Face 发布。本文件是发布说明草稿，
> 不能作为已经发布、可用于生产或已完成许可证审批的证明。
...
- human/legal 许可证清单审批仍为 pending；
- Hugging Face 当前凭据无效，需要在不暴露 token 的前提下重新登录并验证 namespace；
- GitHub 目标仓库尚未创建，本地也没有 remote；
