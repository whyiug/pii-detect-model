# pii-zh community v2 RC1 发布说明

> 本说明由受审离线 renderer 在发布 commit 冻结、模型上传到 private Hugging Face revision 后
> 写入不可变身份，再作为 GitHub draft Release 正文。它不把 RC 描述为生产保证；许可证、私密
> 报告渠道、CI、签名 tag、模型包和远端 revision 由具名外部回执及发布门禁约束。
...
- `hugging_face_immutable_commit: {{HF_IMMUTABLE_SHA}}`
- `hugging_face_immutable_url: {{HF_IMMUTABLE_URL}}`
...
- human/legal 许可证清单由维护者 `whyiug` 于 2026-07-20 审批，无例外；独立自哈希回执绑定
  最终 `LICENSE`、`NOTICE` 和 `THIRD_PARTY_NOTICES.md`；
- GitHub hosted CI 必须精确运行在上述完整 source commit，且签名 tag 必须指向同一 commit；
- GitHub Private Vulnerability Reporting 已启用，但无管理权限账号的独立合成端到端测试尚未完成；
  维护者 `whyiug` 于 2026-07-20 仅对 `0.2.0rc1` 的这一测试门禁做出明确 waiver，发布包绑定
  `private_security_channel_waiver_receipt.json`，不得表述为 tested-channel PASS；
- 该 waiver 不关闭 PVR，不是生产安全保证，也不替代公开 Hugging Face 和发布 GitHub Release 前的
  单独最终确认；
- Hugging Face 文件必须来自重新生成的 publication-successor，并按上述 immutable commit 回读；
- private HF 回读包须再次通过 manifest/checksum 闭包、49 标签合同、公开工件扫描，以及无 GPU、
  禁网、只读文件系统内的 Transformers 加载与有限前向；
- `community-v2-pre-publication-gate-receipt.json` 只表示 draft GitHub Release 与 private HF revision
  已达到等待最终公开确认的状态；它不自行授权可见性切换。

逐项操作、硬门和命令模板见
[`PUBLISHING.md`](https://github.com/whyiug/pii-detect-model/blob/v0.2.0rc1/release/community-v2-rc1/PUBLISHING.md)。
