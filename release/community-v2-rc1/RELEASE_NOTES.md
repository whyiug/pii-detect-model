...
> 状态：已建立 private GitHub/Hugging Face 暂存仓库并推送 release-prep 源码分支，尚未公开或上传
> 模型。本文件是发布说明草稿，不能作为已经公开发布或可用于生产的证明；许可证人工审批由独立
> 自哈希回执证明。
...
- GitHub private repo 已创建，release-prep 分支已推送；source-prep commit `aabb84ac...` 的 hosted
  CI 已通过，最终拟发布 commit 仍须重新运行并绑定成功记录；
- 必须先建立并实测 GitHub 私密安全报告渠道；
