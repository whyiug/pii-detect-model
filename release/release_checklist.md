...
- [x] The final staged package has been inspected to confirm `model.safetensors` is its only weight
  file and contains no pickle, optimizer, scheduler, gradient, cache, raw data, prompt/response,
  customer value, or private log.
- [x] `checksums.txt` has been regenerated and independently verified for every final package file.
- [x] The final CycloneDX SBOM has been regenerated from `uv.lock` and its digest matches both the
  packaged evidence and normalized dependency scan.
- [x] A fresh `pip-audit` run has completed against the release dependency environment; every
  unresolved blocking finding is fixed or has a narrow, approved, unexpired exception with
  compensating controls.
- [x] `scripts/scan_public_artifacts.py` reports no unreviewed secret or likely-real-PII finding for
...
- [x] Offline Transformers 5.13.1 loading uses the serialized fast-tokenizer graph, passes the
  Unicode boundary probe, and preserves source-checkpoint/release-package logit parity.
- [x] `scripts/release_gate.py` exits zero with `status: PASS` for the final artifact from a clean
