...
从仓库根目录运行；先将 `$DATA_ROOT` 设为外部数据根目录，并将 `$LOCAL_PLAN` 指向维护者
本地的规划或溯源文档。该文件不随仓库分发，物化结果只记录文件名与 SHA-256：

```bash
: "${LOCAL_PLAN:?set LOCAL_PLAN to a local planning/provenance document}"
